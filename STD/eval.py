...
#CUDA_VISIBLE_DEVICES=0 python eval.py experiments/seg_detector/ic15_resnet50_deform_thre.yaml --resume ic15_resnet50 --polygon --box_thresh 0.55 --speed

#CUDA_VISIBLE_DEVICES=0 python eval.py experiments/seg_detector/ic15_resnet50_deform_thre.yaml --resume final_g.pth --box_thresh 0.55


#CUDA_VISIBLE_DEVICES=0 python eval.py experiments/seg_detector/ic15_cspresnext50_deform_thre.yaml --resume model_epoch_731_minibatch_30000.pth --box_thresh 0.55
######CUDA_VISIBLE_DEVICES=0 python eval.py experiments/seg_detector/ic15_resnext101_deform_thre.yaml --resume model_epoch_1188_minibatch_297000 --box_thresh 0.55
...
class Eval:
    ...
    def init_model(self):
        model = self.structure.builder.build(self.device)
        #model = self.structure.builder.build( torch.device('cuda'))
        return model
    ...
    def resume(self, model, path):
        ...
        states = torch.load(
            path, map_location=self.device)
        model.load_state_dict(states, strict=False)
        # print("model.module.decoder.down5.0.weight::::::::::::::::::::",model.module.decoder.down5.weight)
        #print(states.keys()) ###model.module.decoder.down5.0.weight
            ...
            for _, data_loader in self.data_loaders.items():
                ...
                for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
                    if self.args['test_speed']:
                       ...
                       #continue
                    pred = model.forward(batch, training=False)
                    #save_image(pred, 'BIN_ima.png')
                    #print("pred-",torch.unique(pred, sorted=True, return_inverse=True))
                    #pred *= 1000000000
                    #print("pred-255----",torch.unique(pred, sorted=True, return_inverse=True))
                    output = self.structure.representer.represent(batch, pred, is_output_polygon=self.args['polygon']) 
                    if not os.path.isdir(self.args['result_dir']):
                        os.mkdir(self.args['result_dir'])
                    self.format_output(batch, output)
