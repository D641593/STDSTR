...
class SegDetectorVisualizer(Configurable):
    ...
    #def demo_visualize(self, image_path, output, words,confs):
...
###########################
#         txtfilename = 'res_' + image_path.split('/')[-1].split('.')[0] + '.txt'
#         txtfilename = os.path.join('demo_results', txtfilename)
#         # print("txtfinemaeeeeee",txtfilename)
#         c = 0
#         with open(txtfilename) as f:
#             for line in f:
#                 coor = line.split(",")
#                 word = str(words[c]).replace("[","").replace("]","").replace("'","")
#                 print("c=========",c," line============",line,"word===========",word)
#                 cv2.putText(pred_canvas, "{}".format(word), (int(coor[0])+1, int(coor[1])+1), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), thickness=2)
#                 c += 1
