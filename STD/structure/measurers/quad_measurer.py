...
class QuadMeasurer(Configurable):
    ...
    def validate_measure(self, batch, output, is_output_polygon=False, box_thresh=0.6):
        ...
        #return self.measure(batch, output, is_output_polygon, box_thresh), [int(self.visualized)]
    ...
    def gather_measure(self, raw_metrics, logger: Logger):
        #print("raw_metrics..........",type(raw_metrics))
...
class QuadMeasurer(Configurable):
    ...
    def measure(self, batch, output, is_output_polygon=False, box_thresh=0.6):
        ...
        pred_polygons_batch = np.array(output[0])
        pred_scores_batch = np.array(output[1])
        for polygons, gts, pred_polygons, pred_scores, ignore_tags in\
                zip(gt_polyons_batch, gt_batch, pred_polygons_batch, pred_scores_batch, ignore_tags_batch):
            gt = [dict(points=polygons[i], ignore=ignore_tags[i], gts=gts[i])
                  for i in range(len(polygons))]
            if is_output_polygon:
                pred = [dict(points=pred_polygons[i])
                        for i in range(len(pred_polygons))]
