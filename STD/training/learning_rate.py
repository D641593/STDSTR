...
class DecayLearningRate(Configurable):
    ...
    def get_learning_rate(self, epoch, step=None):
        print("decay!!!!!!!!!!! epoch = ",epoch,"  self.epochs=============",self.epochs)

        rate = np.power(1.0 - epoch / float(self.epochs + 1), self.factor)
        print("lr ======",self.lr,"  update=========",rate*self.lr)
        return rate * self.lr
...
############cosine decay#################
class cosDecayLearningRate(Configurable):
    lr = State(default=0.001)
    decay_step = State(default=20000)
    epochs = State(default=1200)
    factor = State(default=0.9)
    min_lr = State(default=0.00001)
    ...
    def get_learning_rate(self, epoch, step):
        ...
        rate = 0.1*(1+math.cos((math.pi*(1600+(epoch % 100)/1800))))
        #rate = 0.1*(1+math.cos((math.pi*(step % 800000)/800000)))
        lr = np.max([self.min_lr, rate * self.lr])
        return lr
