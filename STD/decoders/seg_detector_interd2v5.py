...
class InD2v4SegDetector(nn.Module):
    def __init__(self,
        ...
        '''
        super(InD2v4SegDetector, self).__init__()
        self.k = k
        out_channels = 64
        self.serial = serial
        ...
        self.out4 = nn.Conv2d(384, inner_channels, 3, padding=1, bias=bias)
        self.out3 = nn.Conv2d(384, inner_channels, 3, padding=1, bias=bias)
        self.out2 = nn.Conv2d(384, inner_channels, 3, padding=1, bias=bias)
        
        # self.M3d = nn.Sequential(
        #     nn.Conv2d(out_channels, 648, 4, stride=4, bias=bias),
        #     nn.PixelShuffle(3))
        ...
        #     nn.Conv2d(out_channels, 648, 2, stride=2, bias=bias),
        #     nn.PixelShuffle(3))
        ...
        self.M3 = nn.Sequential(
            nn.Conv2d(inner_channels*2, inner_channels*2, 1, bias=bias),
            nn.Conv2d(inner_channels*2, inner_channels, 3, padding=1, bias=bias))
        ...
        self.D1 = nn.Sequential(
            nn.Conv2d(inner_channels, inner_channels , 3, padding=1,stride=2 ,bias=bias)
        )
        ...
        self.binarize = nn.Sequential(
            nn.Conv2d(256*9, inner_channels //
        ...
        if adaptive:
            self.thresh = self._init_thresh(
                    256*9, serial=serial, smooth=smooth, bias=bias)
    ...
    def forward(self, features, gt=None, masks=None, training=False):
        ...
        u1 = self.out5(in5) #256
        out4 = torch.cat([self.up5(u1),in4], dim=1)  # 1/16,256+256=512
        u2 = self.out4(out4) #256
        out3 = torch.cat([self.up4(u2),in3], dim=1)  # 1/8,512
        u3 = self.out3(out3) #256
        out2 = torch.cat([self.up3(u3),in2], dim=1) # 1/4,512
        u4 = self.out2(out2) #256
...
#         print("u4,u3,u2,u1",u4.shape,u3.shape,u2.shape,u1.shape)
        ...
        m3 = self.M3(torch.cat([u4_d, u3_u], dim=1)) #256
        m2 = self.M2(torch.cat([u3_d, u2_u], dim=1)) #256
        m1 = self.M1(torch.cat([u2_d, u1_u], dim=1)) #256
...
#         d3 = self.D3(d2)
        ...
        m3 = F.interpolate(m3, size=c2.size()[2:][0], mode='bilinear', align_corners= True)
        m2 = F.interpolate(m2, size=c2.size()[2:][0], mode='bilinear', align_corners= True)
        m1 = F.interpolate(m1, size=c2.size()[2:][0], mode='bilinear', align_corners= True)
        u3 = F.interpolate(u3, size=c2.size()[2:][0], mode='bilinear', align_corners= True)
        u2 = F.interpolate(u2, size=c2.size()[2:][0], mode='bilinear', align_corners= True)
        u1 = F.interpolate(u1, size=c2.size()[2:][0], mode='bilinear', align_corners= True)
        ...
        d1 = F.interpolate(d1, size=c2.size()[2:][0], mode='bilinear', align_corners= True)
        d2 = F.interpolate(d2, size=c2.size()[2:][0], mode='bilinear', align_corners= True)
#         print("all size-------",u4.shape,m3.shape,u3.shape,m2.shape,u2.shape,m1.shape,u1.shape,d1.shape,d2.shape)
                
        #fuse = torch.cat((p5, p4, p3, p2), 1)
        fuse = torch.cat((u4,m3,u3,m2,u2,m1,u1,d1,d2), 1) #256*9
